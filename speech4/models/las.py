...
class LASModel(object):
  ...
  def create_decoder(self):
    self.decoder_states = []
    self.decoder_states.append(self.tokens)
    ...
    self.create_decoder_attention_layer(attention_states)


  def create_decoder_attention_layer(self, attention_states, scope=None):
    with vs.variable_scope('decoder_layer_%d' % (len(self.decoder_states))):
      ...
      if len(self.decoder_states) == 1:
        self.decoder_states.append(seq2seq.embedding_attention_decoder(
            self.decoder_states[-1], decoder_initial_state, attention_states, rnn_cell.GRUCell(self.decoder_cell_size),
