...
struct ApplyAdadelta<CPUDevice, T> {
  void operator()(const CPUDevice& d, typename TTypes<T>::Flat var,
                  ...
                  typename TTypes<T>::ConstFlat grad) {
    if (DoInline(var.size())) {
      accum_grad = accum_update * decay_rate() + grad.square() * (1 - decay_rate());
      const auto update = (accum_update + epsilon()).sqrt() * (accum_grad + epsilon()).rsqrt() * grad;
      accum_update = accum_update * decay_rate() + update.square() * (1 - decay_rate());
      var -= accum_update * lr();
    } else {
      accum_grad.device(d) = accum_update * decay_rate() + grad.square() * (1 - decay_rate());
      const auto update = (accum_update + epsilon()).sqrt() * (accum_grad + epsilon()).rsqrt() * grad;
      accum_update.device(d) = accum_update * decay_rate() + update.square() * (1 - decay_rate());
 ...
 private:
  ...
  void DoValidate(OpKernelContext* ctx) {
    ...
    OP_REQUIRES(ctx, IsLegacyScalar(lr.shape()),
                errors::InvalidArgument("lr is not a scalar: ",
                                        lr.shape().DebugString()));
    const Tensor& decay_rate = ctx->input(4);
    OP_REQUIRES(ctx, IsLegacyScalar(decay_rate.shape()),
  ...
  void DoCompute(OpKernelContext* ctx) {
    ...
    functor::ApplyAdadelta<Device, T>()(
        device, var.flat<T>(), accum_grad.flat<T>(), accum_update.flat<T>(),
        lr.scalar<T>(), decay_rate.scalar<T>(), epsilon.scalar<T>(),
        grad.flat<T>());
 ...
 public:
  ...
  void Compute(OpKernelContext* ctx) override NO_THREAD_SAFETY_ANALYSIS {
    ...
    if (N > 0) {
      ...
      for (Tindex i = 0; i < N; i++) {
        const Tindex index = indices_vec(i);
        auto ag = accum_grad_flat.template chip<0>(index);
        auto au = accum_update_flat.template chip<0>(index);
        auto g = grad_flat.template chip<0>(i);

        ag = au * au.constant(decay_rate_scalar) + g.square() * g.constant(1 - decay_rate_scalar);
        const auto update = (au + au.constant(epsilon_scalar)).sqrt() * (ag + ag.constant(epsilon_scalar)).rsqrt() * g;
        au = au * au.constant(decay_rate_scalar) + update.square() * update.constant(1 - decay_rate_scalar);
